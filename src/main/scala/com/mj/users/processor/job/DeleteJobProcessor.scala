...
import com.mj.users.mongo.JobDao.jobCollection
import reactivemongo.bson.document

import scala.concurrent.ExecutionContext.Implicits.global
import scala.concurrent.Future

class DeleteJobProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String , jobID : String) => {
      ...
      val result = remove(jobCollection, document("memberID" -> memberID , "jobID" -> jobID))
