...
import com.mj.users.mongo.JobDao.getJobDetailsByID

import scala.concurrent.ExecutionContext.Implicits.global

class GetJobByMemberProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String) => {
      val origin = sender()
      val result = getJobDetailsByID(memberID).map(response =>
        response match {
