...
import com.mj.users.mongo.EventDao.eventCollection
import reactivemongo.bson.document

import scala.concurrent.ExecutionContext.Implicits.global
import scala.concurrent.Future

class DeleteEventProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String , eventID : String) => {
      ...
      val result = remove(eventCollection, document("memberID" -> memberID , "eventID" -> eventID))
