...
import com.mj.users.mongo.UpdateDao.getOneUpdateDetails

import scala.concurrent.ExecutionContext.Implicits.global

class GetOneUpdateProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String , updateID : String) => {
      val origin = sender()
      val result = getOneUpdateDetails(memberID , updateID).map(response =>
        response match {
